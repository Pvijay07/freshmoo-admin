...
const CouponForm = ({ coupon, onSubmit, onCancel }) => {
  ...
  const [discountType, setDiscountType] = useState(coupon ? coupon.discount_type : 'percentage');
  const [maxDiscount, setMaxDiscount] = useState(coupon ? coupon.max_discount : '');
  const [minOrderAmount, setMinOrderAmount] = useState(coupon ? coupon.min_order_amount : '');
